...
type Attachment struct {
	...
	// FORGIVE ME: I could not figure out how to use string signature
	SignatureBuff  bytes.Buffer `json:"-"`
}
...
func (att *Attachment) Encrypt(ciphertext io.Writer, signature io.WriteCloser, signed *openpgp.Entity) (cleartext io.WriteCloser, err error) {
	config := &packet.Config{}
	...
	return symmetricallyEncrypt(ciphertext, signature, att.unencryptedKey, signer, hints, config)
}
...
func (c *Client) CreateAttachment(att *Attachment, r io.Reader) (created *Attachment, err error) {
	...
	go func() {
		...
		// TODO use Signature: string here
		if att.SignatureBuff.Len() != 0 {
			if w, err := mw.CreateFormFile("Signature", "blob"); err != nil {
				pw.CloseWithError(err)
				return
			} else if _, err := io.Copy(w,  bufio.NewReader(&att.SignatureBuff)); err != nil {
				pw.CloseWithError(err)
				return
			}
		}
